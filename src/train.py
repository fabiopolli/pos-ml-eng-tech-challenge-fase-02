...
def load_processed_data():
    ...
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(
            f"Arquivo não encontrado: {DATA_PATH}. Rode o data_preparation primeiro."
        )
...
def train_popularity_baseline(df, top_n=10):
    ...
    top_products = popularity.sort_values(
        by="interactions_count", ascending=False
    ).head(top_n)
...
def train_top_rated_baseline(df, min_reviews=5, top_n=10):
    ...
    top_rated = (
        ratings[ratings["count"] >= min_reviews]
...
def run_experiment(run_name, top_n_param, min_reviews_param):
    with mlflow.start_run(run_name=run_name):
        ...
        rate_recs = train_top_rated_baseline(
            df, min_reviews=min_reviews_param, top_n=top_n_param
        )
        ...
        recs_df = pd.DataFrame(
            {
                "popularity_baseline": pd.Series(pop_recs),
                "top_rated_baseline": pd.Series(rate_recs),
            }
        )
        recs_df.to_csv(
            "data/processed/temporary_baseline_recommendations.csv", index=False
        )
